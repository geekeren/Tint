...
if (!function_exists('tin_load')) {
    function tin_load()
    {
        ...
        load_template(THEME_DIR . '/functions/widgets/tin-subscribe-mp.php');
...
if (!function_exists('tin_sidebars')) {
    ...
    function tin_sidebars()
    {
        ...
        if (ot_get_option('footer-widgets') >= '1') {
            register_sidebar(array('name' => 'Footer 1', 'id' => 'footer-1', 'description' => __("底部多列边栏1", 'tinection'), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span class=widget-title>', 'after_title' => '</span></h3>'));
        }
        if (ot_get_option('footer-widgets') >= '2') {
            register_sidebar(array('name' => 'Footer 2', 'id' => 'footer-2', 'description' => __("底部多列边栏2", 'tinection'), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span class=widget-title>', 'after_title' => '</span></h3>'));
        }
        if (ot_get_option('footer-widgets') >= '3') {
            register_sidebar(array('name' => 'Footer 3', 'id' => 'footer-3', 'description' => __("底部多列边栏3", 'tinection'), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span class=widget-title>', 'after_title' => '</span></h3>'));
        }
        if (ot_get_option('footer-widgets') >= '4') {
            register_sidebar(array('name' => 'Footer 4', 'id' => 'footer-4', 'description' => __("底部多列边栏4", 'tinection'), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span class=widget-title>', 'after_title' => '</span></h3>'));
        }
        if (ot_get_option('footer-widgets-singlerow') == 'on') {
            register_sidebar(array('name' => 'Footer row', 'id' => 'footer-row', 'description' => __("底部通栏", 'tinection'), 'before_widget' => '<div id="%1$s" class="widget %2$s">', 'after_widget' => '</div>', 'before_title' => '<h3><span class=widget-title>', 'after_title' => '</span></h3>'));
        }
    }
...
function pagenavi($before = '', $after = '', $p = 2)
{
    ...
    echo $before;
    if ($paged > 1)
        p_link($paged - 1, __('上一页', 'tinection'), '<span class="pg-item pg-nav-item pg-prev">', __('上一页', 'tinection'));
    if ($paged > $p + 1)
    ...
    if ($tags) {
        ...
        if ($my_query->have_posts()) {
            ...
            while ($my_query->have_posts()) : $my_query->the_post();
                $related .= '<li style="list-style:circle;font-size:13px;"><a style="color:#1cbdc5;font-size:13px;font-family:微软雅黑,Microsoft Yahei;" href="';
                $related .= get_permalink();
                $related .= '" rel="bookmark" title="';
                $related .= get_the_title($post->ID);
                $related .= '">';
                $related .= get_the_title($post->ID);
                $related .= ' ( ';
                $related .= get_comments_number($post->ID);
                $related .= ' ) ';
                $related .= '</a></li>';endwhile;
            $related .= '</ul>';
        }
    }
    return $related;
...
function tin_post_meta($special = 0)
{
    ...
    if (is_singular()) { ?>
        <div id="single-meta">
            ...
            <span class="single-meta-time"><i
                    class="fa fa-calendar">&nbsp;</i><?php echo timeago(get_gmt_from_date(get_the_time('Y-m-d G:i:s'))) ?></span>
            <?php if (is_single()) { ?><span class="single-meta-category"><i
                class="fa fa-folder-open">&nbsp;</i><?php the_category(' ', ''); ?></span><?php } ?>
            <?php if (current_user_can('level_7')) { ?><span class="single-meta-edit"><i
                class="fa fa-edit">&nbsp;</i><?php edit_post_link(__(' 编辑 ', 'tinection')); ?></span><?php } ?>
            <span class="single-meta-comments"><?php if (comments_open()): ?>|&nbsp;&nbsp;<i
                    class="fa fa-comments"></i>&nbsp;<a href="#"
                                                        class="commentbtn"><?php comments_number(__('抢沙发', 'tinection'), __('1 条评论', 'tinection'), __('% 条评论', 'tinection')); ?></a><?php else: ?>|&nbsp;
                    <i class="fa fa-comments"></i><?php _e(' 评论关闭', 'tinection'); ?><?php endif; ?></span>
            <span class="single-meta-views"><i
                    class="fa fa-fire"></i>&nbsp;<?php echo get_tin_traffic('single', get_the_ID()); ?>&nbsp;</span>
        </div>
    <?php } elseif (is_search() || (is_home() && $thelayout == 'blog') || (is_category() && $special == 0) || is_tag() || is_date()) { ?>
        <div class="meta">
            <?php if (!is_category()) { ?><span class="postlist-meta-cat"><i
                class="fa fa-bookmark"></i><?php the_category(' ', false); ?></span><?php } ?>
            <span class="postlist-meta-time"><i
                    class="fa fa-calendar"></i><?php echo timeago(get_gmt_from_date(get_the_time('Y-m-d G:i:s'))); ?></span>
            <span class="postlist-meta-views"><i
                    class="fa fa-fire"></i><?php _e("浏览: ","tinection");echo get_tin_traffic('single', get_the_ID()); ?></span>
            <span class="postlist-meta-comments"><?php if (comments_open()): ?><i class="fa fa-comments"></i><a
                    href="<?php comments_link(); ?>"><?php comments_number(__('<span>评论: </span>0', 'tinection'), __('<span>评论: </span>1', 'tinection'), __('评论: </span>%', 'tinection')); ?></a><?php endif; ?></span>
        </div>
    <?php } elseif ((is_home()) || (is_category() && $special == 1)) { ?>
        <div class="postlist-meta">
            <div class="postlist-meta-time"><i
                    class="fa fa-calendar"></i>&nbsp;<?php echo date('Y-m-j', get_the_time('U')); ?></div>
            <div class="postlist-meta-views" style="float:right;"><i
                    class="fa fa-fire"></i>&nbsp;<?php echo get_tin_traffic('single', get_the_ID()); ?></div>
    ...
    <?php } else { ?>
        <div id="single-meta">
            ...
            <span class="single-meta-comments"><?php if (comments_open()): ?>|&nbsp;&nbsp;<i
                    ...
                    href="<?php comments_link(); ?>"><?php comments_number(__('0 条评论', 'tinection'), __('1 条评论', 'tinection'), __('% 条评论', 'tinection')); ?></a><?php else: ?>|&nbsp;
                    <i class="fa fa-comments"></i><?php _e(' 评论关闭', 'tinection'); ?><?php endif; ?></span>
            <span class="single-meta-views"><i
                    class="fa fa-fire"></i>&nbsp;<?php echo get_tin_traffic('single', get_the_ID()); ?>&nbsp;</span>
        </div>
...
function timeago($ptime)
{
    ...
    $interval = array(
        12 * 30 * 24 * 60 * 60 => __('年前','tinection').' (' . date('Y-m-d', $ptime) . ')',
        30 * 24 * 60 * 60 => __('个月前', 'tinection').' (' . date('m-d', $ptime) . ')',
        7 * 24 * 60 * 60 => __('周前', 'tinection').' (' . date('m-d', $ptime) . ')',
        24 * 60 * 60 => __('天前', 'tinection'),
        60 * 60 => __('小时前', 'tinection'),
        60 => __('分钟前', 'tinection'),
        1 => __('秒前', 'tinection')
    );
...
/* 获取头像
/* ------------ */
function tin_get_avatar($id, $size = '40', $type = '', $name = '')
{
    if ($type === 'qq') {
        $O = array(
        ...
        $avatar_url = NameAvatar::url($name, $size);
    } else {
...
function tin_add_register_captcha()
{
    ...
    <p style="overflow:hidden;">
        <label for="tin_captcha">验证码<br>
            <input type="text" name="tin_captcha" id="tin_captcha" aria-describedby="" class="input" value="" size="20"
                   style="float:left;margin-right:10px;width:190px;">
            <img src="<?php echo $captcha; ?>" class="captcha_img inline" title="点击刷新验证码"
                 onclick="this.src='<?php echo $captcha; ?>';" style="float:right;margin-top: 5px;"></label>
    </p>
    <?php
}
